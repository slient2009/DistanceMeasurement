...
#define LOG_DISTANCE_GAP 128
#define LOG_DISTANCE_TIME_GAP 100
...
static void log_distance(double d, int type, int the_id){
  // fprintf(distance_log_file, "%.2f %.2f %.2f | %llu\n", min_distance, d, max_distance, total_execs);
  if(type==2){
...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  if (*total_count > 0)
    ...
    cur_distance =   (((unsigned int)(1<<31)-1) / (double) (*total_distance)) * (double) (*total_count);     //
  else
    cur_distance = -1.0;

  log_distance(cur_distance, 1, 0);
  ...
  if (*total_count > 0) {
    cur_distance = (double) (*total_distance) / (double) (*total_count);
    cur_distance =   (((1<<31)-1) / (double) (*total_distance)) * (double) (*total_count);     //
  else
    cur_distance = -1.0;
...
static u8* describe_op(u8 hnb) {
  ...
  sprintf(ret + strlen(ret), ",dis:%.2f", cur_distance);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    fn = alloc_printf("%s/queue/id:%06u,%llu,%s", out_dir, queued_paths,
                      ...
                      describe_op(hnb));
    log_distance(cur_distance, 3, queued_paths);
...
keep_as_crash:
      ...
      log_distance(cur_distance, 4, queued_paths);
...
EXP_ST void setup_dirs_fds(void) {
  ...
  /* log_distance file.*/
  tmp = alloc_printf("%s/distance_log", out_dir);
  fd = open(tmp, O_WRONLY | O_CREAT | O_EXCL, 0600);
  if (fd < 0) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  distance_log_file = fdopen(fd, "w");
  if(!distance_log_file) PFATAL("fdopen() failed");

  fprintf(distance_log_file, "# type | min_distance cur_distance max_distance | total_execs | offtime | id:xxx\n");
  last_distance_log_seq = 0;
}
