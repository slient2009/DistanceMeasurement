  ...
  if (!TargetsFile.empty()) {
    ...
    if (cf.is_open()) {
      ...
      while (getline(cf, line)) {
        ...
        int bb_dis = (int) ( ((unsigned int)(1<<31)-1) / atof(line.substr(pos + 1, line.length()).c_str()) );
        // 
        // sigma(MAXN/Xi)
        // harmonic = N/sigma(1/Xi...) = (MAXN*N)/sigma(MAXN/Xi...)
        ...
        */
        ...
        if (distance >= 0) {
          ...
          Value *MapDistPtr = IRB.CreateBitCast(
              IRB.CreateGEP(MapPtr, MapDistLoc), LargestType->getPointerTo());// MapDistPtw
          ...
          Value *IncrDist = IRB.CreateAdd(MapDist, Distance);//MapDistLoc
          ...
          Value *IncrCnt = IRB.CreateAdd(MapCnt, One);  // MapCntLoc
