  ...
  if (!TargetsFile.empty()) {
    ...
    if (cf.is_open()) {
      ...
      while (getline(cf, line)) {
        ...
        bb_to_dis.emplace(bb_name, bb_dis); // map 
        basic_blocks.push_back(bb_name);    // vector
  ...
  if (is_aflgo_preprocessing) {
    ...
    for (auto &F : M) {// 
      ...
      for (auto &BB : F) { // 
        ...
        for (auto &I : BB) {
          getDebugLoc(&I, filename, line); // filename
          ...
          if (auto *c = dyn_cast<CallInst>(&I)) { // c
        ...
        if (!bb_name.empty()) {
          ...
          */
          ...
          if (!BB.hasName()) { 
            ...
            // bbnames << "BB has no Name:" + BB.getName().str() << "\n"; // 
          }
    ...
    for (auto &F : M) {
      ...
      for (auto &BB : F) {
        ...
        */
        ...
        IRBuilder<> IRB(&(*IP)); // IR Builder， 

        if (AFL_R(100) >= inst_ratio) continue; // define AFL_R(x) (random() % (x))
        ...
        StoreInst *Store =
            IRB.CreateStore(ConstantInt::get(Int32Ty, cur_loc >> 1), AFLPrevLoc); // cur_loc
        ...
        */
        ...
        if (distance >= 0) {
          ...
          Value *MapDistPtr = IRB.CreateBitCast(
              IRB.CreateGEP(MapPtr, MapDistLoc), LargestType->getPointerTo());// MapDistPtr
          LoadInst *MapDist = IRB.CreateLoad(MapDistPtr); 
          MapDist->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));

          //
          // Value *tmp = IRB.CreateAdd(Distance, ConstantInt::get(LargestType, 1000));
          Value *NewDistance = IRB.CreateSelect(IRB.CreateICmpSLT(Distance, MapDist), Distance, MapDist);

          // 
          IRB.CreateStore(NewDistance, MapDistPtr)
            ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
