        ...
        while(len(q)>0):
            ...
            if(cur_bb_name in BBcall_bb2f):
                for callee_func_name in BBcall_bb2f[cur_bb_name]:
                    if(callee_func_name not in func_name_to_node_id): # calllgraph，
                        continue
                    ...
                    try:
                        ...
                        if('weight' not in CG.edges[src_node , dst_node]):
                            ...
                            if(cur_dist < CG.edges[src_node , dst_node]['weight']):
                                CG.edges[src_node , dst_node]['weight'] = cur_dist
                                # print(src_node , dst_node, '->', cur_dist)
                    except KeyError: # bbcallfunction call?llvmcallgraph，xyzxyz.123。
...
def construct_callgraph(args, binaries):
    ...
    for binary in binaries:
        opt_callgraph(args, binary) # optCG
    ...
    if fuzzer:
        ...
        # fuzzer_name，cg，
    print("===", dot_files)
    adjust_callgraph(callgraph_out, dot_files.glob("cfg.*.dot"), args.temporary_directory / "BBcalls.txt")  # func.cfg.dotBBcalls.txtCallgraph。
    next_step(args)

# CGfunction distance，txt
...
def main():
    ...
    if not STEP:
        construct_callgraph(args, binaries), # binariesCG
