    ...
    for (vertex_desc n : find_nodes(G, name)) {
        ...
        if (is_cg) {
            for (vertex_desc t : targets) {
                ...
                // d += 1.0 / (1.0 + static_cast<double>(shortest)); // harmonic mean
                d += 1.0 + static_cast<double>(shortest); // arithmetic mean, one function distance should be preserved
                ++i;
            }
        } else {
            for (auto &bb_d_entry : bb_distance) {
                ...
                for (auto t : find_nodes(G, bb_d_entry.first)) {
                    auto shortest = distances[t];           // shortest distance from n to t
                    if (shortest == 0 and n != t) continue; // not reachable
                    // di += 1.0 / (1.0 + 10 * bb_d_entry.second + static_cast<double>(shortest)); // harmonic mean
                    // di += (10 * bb_d_entry.second + static_cast<double>(shortest)); // standard arithmetic mean
                    di += (1.0 + 10 * bb_d_entry.second + static_cast<double>(shortest)); // arithmetic mean, but with 1.0+ version
                    ++ii;
                }
                if (ii != 0) {
        ...
        double tmp = d / static_cast<double>(i); // arithmetic mean
        if (d != 0 and (distance == -1 or distance > tmp)) { // 如果保留1.0+操作，那么就要检查d>0了。这里distance默认为-1，那么0可以作为一个真实值。
        // if (i>0 && (distance == -1 or distance > tmp)) { // 如果要使用标准的、正确的算术平均值，就不需要判断d>0，因为到达了target之后距离真的就是0了
            distance = tmp;
        }
    }
