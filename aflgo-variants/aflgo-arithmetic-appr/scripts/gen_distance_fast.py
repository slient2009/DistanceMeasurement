...
# merge_callgraph就是用networkx的update方法反复去更新cg，输出路径为callgraph_out
...
def opt_callgraph(args, binary):
    ...
    cmd = ["opt", "-dot-callgraph", f"{binary}",
           "-callgraph-dot-filename-prefix", prefix, # 这个prefix是在干啥啊？
           "-o", "/dev/null"]
    log_p = args.temporary_directory / f"step{STEP}.log"
...
def construct_callgraph(args, binaries):
    ...
    # 这里的fuzzer应该指的是要测试的目标程序
    # 如果指定了fuzzer，就只为fuzzer生成CG，否则就要为目录下面所有的程序生成CG
    if fuzzer:
        tmp = next(args.binaries_directory.glob(f"{fuzzer.name}.0.0.*.bc"))
        binaries = [tmp]

    for binary in binaries:
        opt_callgraph(args, binary) # 这个函数会调用opt工具来生成当前程序的CG
        ...
        remove_repeated_lines(temp, callgraph) # 去除重边
        temp.unlink()
    ...
    if fuzzer:
        ...
        # 没有指定fuzzer_name，要生成很多cg，并且最后要把它们合并起来
    next_step(args)

# 这个函数在一个巨大的CG上面计算function distance，依赖的数据来自若干txt文件
...
def main():
    ...
    STEP = get_resume(args) # 支持从上次计算的断点处继续计算
    if not STEP:
        construct_callgraph(args, binaries), # 这个函数会对binaries中所有的程序都生成对应的CG
