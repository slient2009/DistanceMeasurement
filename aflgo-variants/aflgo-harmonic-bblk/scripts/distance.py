    ...
    if is_cg:
      for t in targets: # 
        try:
          ...
          # log_file.write("shortest(%s, %s) found with %d\n" % (n, t, shortest))
          d += 1.0 / (1.0 + shortest) # 
          i += 1
        ...
        for t in find_nodes(t_name): # 
          try:
            if(n==t):
              ...
              shortest = nx.dijkstra_path_length(G, n, t)
            di += 1.0 / (1.0 + bb_d + shortest)
            ii += 1
            # log_file.write("%s -> %s -> targets : %f\n" % (G.nodes[n]['label'], G.nodes[t]['label'], shortest))
          except nx.NetworkXNoPath:
            pass
        if ii != 0:
          d += di / ii
...
if __name__ == '__main__':
  ...
  if not is_cg :
    ...
    if args.cg_distance is None:
      ...
      print ("Loading cg_distance for function '%s'.." % caller) # caller
